 ...
 * 
 * SVN : $URL: http://svn.centreon.com/branches/centreon-2.1/www/include/views/graphs/generateGraphs/generateODSImageZoom.php $
 * SVN : $Id: generateODSImageZoom.php 10652 2010-06-30 10:21:07Z jmathis $
 * 
 */ 
	...
	if (!$session->numRows()){
		...
		/*
		 * don't "pass" if user has selected metrics
		 */
		$pass = 1;
		$DBRESULT =& $pearDBO->query("SELECT * FROM metrics WHERE `index_id` = '".$_GET["index"]."' AND `hidden` = '0' ORDER BY metric_name");
		...
		while ($metric =& $DBRESULT->fetchRow()) {
			/* ... not sure if this ever worked (nfilus) */
			if ( (isset($_GET["metric"]) && !isset($_GET["metric"][$metric["metric_id"]]))
				&& !isset($_GET["index_id"])
				&& !$pass )
				continue;
