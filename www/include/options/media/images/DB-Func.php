 ...
 * 
 * SVN : $URL: http://svn.centreon.com/branches/centreon-2.1/www/include/options/media/images/DB-Func.php $
 * SVN : $Id: DB-Func.php 10605 2010-06-23 17:02:30Z nfilus $
 * 
 */
	...
	function processFileUpload($fileform) {
			$fDataz =& $fileform->getValue();
			if (stristr($fDataz["type"], "image") && !is_file("./img/media/".$dir_alias["dir_alias"]."/".str_replace(" ", "_", $fDataz["name"])))	{
				# Move image in 'media' directory
				$fileform->moveUploadedFile("./img/media/");				
				$new_file_name = str_replace(" ", "_", $fDataz["name"]);	
				rename("./img/media/".$fDataz["name"], "./img/media/".$new_file_name);
	...
	function insertImg($file = NULL, $file1 = NULL, $file2 = NULL, $file3 = NULL, $file4 = NULL, $path = NULL)	{
		...
		if ($file1)
			processFileUpload($file1);
		if ($file2)
			processFileUpload($file2);
		if ($file3)
			processFileUpload($file3);
		if ($file4)
			processFileUpload($file4);
