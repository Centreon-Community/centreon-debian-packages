 ...
 * 
 * SVN : $URL: http://svn.centreon.com/branches/centreon-2.1/www/include/options/accessLists/menusACL/formMenusAccess.php $
 * SVN : $Id: formMenusAccess.php 10473 2010-05-19 21:25:56Z jmathis $
 * 
 */
	...
	while ($topo1 =& $DBRESULT1->fetchRow())	{
		...
		$b = 0;
	 	$DBRESULT2 =& $pearDB->query("SELECT topology_id, topology_page, topology_name, topology_parent FROM topology WHERE topology_parent = '".$topo1["topology_page"]."' AND topology_show = '1' ORDER BY topology_order");
		while ($topo2 =& $DBRESULT2->fetchRow())	{
			...
			while ($topo3 =& $DBRESULT3->fetchRow()){
				...
				$d = 0;
			 	$DBRESULT4 =& $pearDB->query("SELECT topology_id, topology_name, topology_parent FROM topology WHERE topology_parent = '".$topo3["topology_page"]."' AND topology_page IS NOT NULL  ORDER BY topology_order");
