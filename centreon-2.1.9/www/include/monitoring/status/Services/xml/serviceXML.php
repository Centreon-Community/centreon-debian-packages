 ...
 *
 * SVN : $URL: http://svn.centreon.com/branches/centreon-2.1/www/include/monitoring/status/Services/xml/serviceXML.php $
 * SVN : $Id: serviceXML.php 10703 2010-07-13 08:29:34Z shotamchay $
 *
 */
	...
	if ($hostgroups) {
		$rq1 .= ", ".$ndo_base_prefix."hostgroup_members hm ";
	}
	...
	if ($hostgroups && $o != "meta") {
		...
		$rq1 .= " (SELECT hostgroup_id FROM ".$ndo_base_prefix."hostgroups WHERE alias LIKE '".$hostgroups."')";
	}
	...
	$rq3 = 	"SELECT 1 FROM ".$ndo_base_prefix."servicestatus WHERE no.object_id = ns.service_object_id  ";
	...
	$rq2 = 	"SELECT DISTINCT no.name1 as host_name, no.object_id, no.name2 as service_description, " .
			...
			"	AND EXISTS ($rq3)" .
			"	) A, " .
		 	"".$ndo_base_prefix."servicestatus nss WHERE A.object_id = nss.service_object_id $rq_state $rq_sorte $rq_limit ";
	 ...
	 */
	if ($hostgroups) {
		$hgCondition = " AND no.name1 IN (SELECT no.name1 FROM ".$ndo_base_prefix."hostgroup_members hm, ".$ndo_base_prefix."objects no WHERE no.object_id = hm.host_object_id AND hm.hostgroup_id IN
						 (SELECT hostgroup_id FROM ".$ndo_base_prefix."hostgroups WHERE alias LIKE '".$hostgroups."')) ";
	} else {
		$hgCondition = "";
	}
	...
	if ($is_admin) {
		...
		if ($o == "svc_unhandled") {
            $rq_pagination .= ", " . $ndo_base_prefix."hoststatus hs, ".$ndo_base_prefix."services s ";
		}
        $rq_pagination .= " WHERE no.object_id = nss.service_object_id $rq_state $instance_filter AND no.name1 NOT LIKE '_Module_%' $hgCondition $searchHost $searchService ";
        if ($o == "svc_unhandled") {
            $rq_pagination .= " AND nss.service_object_id  = s.service_object_id
		...
		$rq_pagination = " SELECT count(DISTINCT UPPER(CONCAT(no.name1,';', no.name2))) " .
						 " FROM ".$ndo_base_prefix."objects no, ".$ndo_base_prefix."servicestatus nss, centreon_acl ";
	    if ($o == "svc_unhandled") {
            $rq_pagination .= ", " . $ndo_base_prefix."hoststatus hs, ".$ndo_base_prefix."services s ";
		}
		$rq_pagination .= " WHERE no.object_id = nss.service_object_id $rq_state $instance_filter AND no.name1 NOT LIKE '_Module_%' $hgCondition $searchHost $searchService $ACLCondition";
        if ($o == "svc_unhandled") {
            $rq_pagination .= " AND nss.service_object_id  = s.service_object_id
            				    AND s.host_object_id = hs.host_object_id
            				    AND hs.scheduled_downtime_depth = 0
	...
	while ($ndo =& $DBRESULT_NDO2->fetchRow()) {
		if (isset($host_status[$ndo["host_name"]])){
			...
			if ($host_prev == $ndo["host_name"]){
				...
				$buffer->writeAttribute("hnl", urlencode($ndo["host_name"]));
			...
			$buffer->writeElement("sdl", urlencode($ndo["service_description"]));
			...
			$buffer->writeElement("po", $ndo["plugin_output"], 1);
