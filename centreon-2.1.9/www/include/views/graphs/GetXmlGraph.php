 ...
 * SVN : $Id: GetXmlGraph.php 10605 2010-06-23 17:02:30Z nfilus $
 * 
 */
...
//		$split = 0;
		...
		if ($type == "HS" || $type == "MS"){
			...
			if ($msg_error == 0)	{
				if (isset($_GET["metric"]) && $pass){
					$DBRESULT =& $pearDB->query("DELETE FROM `ods_view_details` WHERE index_id = '".$index."' AND `contact_id` = '".$contact_id."'");
		...
		if ($type == "SS" || $type == "SM"){
			...
			if (isset($_GET["metric"]) && $pass){
				$DBRESULT =& $pearDB->query("DELETE FROM `ods_view_details` WHERE index_id = '".$id."' AND `contact_id` = '".$contact_id."'");
				foreach ($metrics_active as $key => $metric){
