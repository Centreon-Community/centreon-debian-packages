 ...
 * 
 * SVN : $URL: http://svn.centreon.com/branches/centreon-2.1/www/include/views/graphs/generateGraphs/generateODSImage.php $
 * SVN : $Id: generateODSImage.php 10715 2010-07-19 16:37:22Z jmathis $
 * 
 */ 
	...
	function legendcmp($item1, $item2) {
	    return strnatcmp($item1["legend"], $item2["legend"]);
	}
	...
	if (!$session->numRows()){
		...
		/*
		 * copy all metrics from selected service instance to an array
		 */
		$DBRESULT =& $pearDB->query("SELECT `metric_id` FROM `ods_view_details` WHERE `index_id` = '".$svc_instance."' AND `contact_id` = '".$session_value["user_id"]."'");
		while ($metric =& $DBRESULT->fetchRow()){
			$metrics_activate[$metric["metric_id"]] = $metric["metric_id"];
		}
		$DBRESULT->free();
		 ...
		 */
		$metrics = array();
		$selector = "index_id = '".$svc_instance."'";
		...
		while ($metric =& $DBRESULT->fetchRow()) {
			if (isset($_GET["metric"]) && isset($_GET["metric"][$metric["metric_id"]]))
				continue;
			if (isset($metrics_activate) 
				&& !(isset($metrics_activate[$metric["metric_id"]]) && $metrics_activate[$metric["metric_id"]])	)
				continue;
