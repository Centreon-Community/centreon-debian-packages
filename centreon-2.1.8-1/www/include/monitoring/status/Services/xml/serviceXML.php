 ...
 * SVN : $Id: serviceXML.php 10473 2010-05-19 21:25:56Z jmathis $
 * 
 */
	...
	if ($is_admin) {
		$rq_pagination ="SELECT count(DISTINCT UPPER(CONCAT(no.name1,';', no.name2))) " .
						...
						"WHERE no.object_id = nss.service_object_id $rq_state $instance_filter AND no.name1 NOT LIKE '_Module_%' $hgCondition $searchHost $searchService ";
	} else {
		$rq_pagination ="SELECT count(DISTINCT UPPER(CONCAT(no.name1,';', no.name2))) " .
						"FROM ".$ndo_base_prefix."objects no, ".$ndo_base_prefix."servicestatus nss, centreon_acl " .
						"WHERE no.object_id = nss.service_object_id $rq_state $instance_filter AND no.name1 NOT LIKE '_Module_%' $hgCondition $searchHost $searchService $ACLCondition";
	}
