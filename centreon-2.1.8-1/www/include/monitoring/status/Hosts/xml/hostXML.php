 ...
 */
	...
	while ($ndo =& $DBRESULT_NDO1->fetchRow()){
		...
		if ($ndo["notes"] != "") {
			$buffer->writeElement("hnn", $hostObj->replaceMacroInString($ndo["name1"], str_replace("\$HOSTNAME\$", $ndo["host_name"], str_replace("\$HOSTADDRESS\$", $ndo["address"], $ndo["notes_url"]))));
		} else {
			$buffer->writeElement("hnn", "none");			
		}
		
		if ($ndo["notes_url"] != "") {
			$buffer->writeElement("hnu", $hostObj->replaceMacroInString($ndo["name1"], str_replace("\$HOSTNAME\$", $ndo["host_name"], str_replace("\$HOSTADDRESS\$", $ndo["address"], str_replace("\$HOSTNOTES\$", $ndo["notes"], $ndo["notes_url"])))));
		} else {
